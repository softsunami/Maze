...
TEST(Cave, NegativeRows) {
  Cave cave = Cave::GetInstance();
  EXPECT_ANY_THROW(
      cave.UploadCaveFromFile("./tests/materials/caves/negative_rows.txt"));
}

TEST(Cave, NegativeColumns) {
  Cave cave = Cave::GetInstance();
  EXPECT_ANY_THROW(
      cave.UploadCaveFromFile("./tests/materials/caves/negative_cols.txt"));
}

TEST(Cave, BadData) {
  Cave cave = Cave::GetInstance();
  EXPECT_ANY_THROW(
      cave.UploadCaveFromFile("./tests/materials/caves/bad_data.txt"));
}
