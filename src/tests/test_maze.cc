...
TEST(Maze, NegativeRows) {
  Maze maze = Maze::GetInstance();
  EXPECT_ANY_THROW(
      maze.UploadMazeFromFile("./tests/materials/mazes/negative_rows.txt"));
}

TEST(Maze, NegativeColumns) {
  Maze maze = Maze::GetInstance();
  EXPECT_ANY_THROW(
      maze.UploadMazeFromFile("./tests/materials/mazes/negative_cols.txt"));
}

TEST(Maze, BadRightWalls) {
  Maze maze = Maze::GetInstance();
  EXPECT_ANY_THROW(
      maze.UploadMazeFromFile("./tests/materials/mazes/bad_right_walls.txt"));
}

TEST(Maze, BadBottomWalls) {
  Maze maze = Maze::GetInstance();
  EXPECT_ANY_THROW(
      maze.UploadMazeFromFile("./tests/materials/mazes/bad_bottom_walls.txt"));
}
...
TEST(Maze, Pathfinder2) {
  Maze maze = Maze::GetInstance();
  maze.UploadMazeFromFile("./tests/materials/mazes/test_pathfinder.txt");
  EXPECT_EQ(maze.FindPath({7, 1}, {-1, 0}).empty(),
