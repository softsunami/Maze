...
MazePainter::~MazePainter() { delete painter; }

inline bool MazePainter::WallExists_(long cell) { return cell == 1; }
...
void MazePainter::SetPath(std::vector<Point> path) { path_ = path; }
...
void MazePainter::PaintSquare_(QPoint& position) {
  ...
  painter->drawRect(x, y, cell_width_ - margin_width * 2,
                    cell_height_ - margin_height * 2);
}
...
void MazePainter::PaintPath_() {
  if (path_.size() > 1 && left_btn_pressed_ && right_btn_pressed_) {
    ...
    double from_x =
        (static_cast<double>(from.x) * cell_height_) + margin_height;
...
void MazePainter::mousePressEvent(QMouseEvent* event) {
  ...
  if (left_btn_pressed_ && right_btn_pressed_) {
    ...
    emit FindPath(Point{cell_y_left, cell_x_left},
                  Point{cell_y_right, cell_x_right});
  }
}
