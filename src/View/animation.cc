...
void MainWindow::SwitchApplicationTab_(QAbstractButton *button) {
  ...
  if (ui_->tabWidgetMazeNCave->currentIndex() != index) {
    ui_->tabWidgetMazeNCave->setCurrentIndex(
        (ui_->tabWidgetMazeNCave->currentIndex() + 1) % 2);
    ui_->tabWidgetDisplay->setCurrentIndex(
        (ui_->tabWidgetDisplay->currentIndex() + 1) % 2);
