...
class Generator<kMaze> final {
 ...
 private:
  ...
  void AddBottomWalls_(std::size_t row) {
    for (std::size_t i = 0; i < maze_.cols; ++i) {
      if (RandomBool_() == true &&
          GetNumberOfCellsWithoutBottomWall_(line_[i], row) > 1) {
  ...
  std::size_t GetNumberOfCellsWithoutBottomWall_(std::size_t set_number,
                                                 std::size_t row) {
    std::size_t count = 0;

    for (std::size_t i = 0; i < maze_.cols; ++i) {
      if (line_[i] == static_cast<int>(set_number) &&
          maze_.bottom_walls[row][i] == false) {
        ++count;
      }
    }
...
class Generator<kCave> final {
 public:
  static CaveType Generate(unsigned int rows, unsigned int cols,
    ...
    for (int i = 0; i < alive_cell_count; ++i)
      flat_matrix[i] = Cell::kAliveCell;
