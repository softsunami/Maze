...
PathFinder::PathFinder(const MazeType& maze, Point from, Point to)
    : maze_(maze), from_(from), to_(to), wave_number_{0}, empty_value_{10000} {
  old_wave_ = {from};
...
bool PathFinder::IsValidPoints_(Point from, Point to) {
  return !(from.x >= static_cast<int>(maze_.rows) ||
           from.y >= static_cast<int>(maze_.cols) || from.x < 0 || from.y < 0 ||
           to.x >= static_cast<int>(maze_.rows) ||
           to.y >= static_cast<int>(maze_.cols) || to.x < 0 || to.y < 0);
}
...
std::vector<Point> PathFinder::MakePath_() {
  ...
  while (length_map_[row][col] != 0) {
    if (col - 1 >= 0 &&
        length_map_[row][col - 1] == length_map_[row][col] - 1 &&
        maze_.right_walls[row][col - 1] == 0) {
      --col;
    } else if (col <= x_size - 1 &&
               length_map_[row][col + 1] == length_map_[row][col] - 1 &&
               maze_.right_walls[row][col] == 0) {
      ++col;
    } else if (row - 1 >= 0 &&
               length_map_[row - 1][col] == length_map_[row][col] - 1 &&
               maze_.bottom_walls[row - 1][col] == 0) {
      --row;
    } else if (row <= y_size - 1 &&
               length_map_[row + 1][col] == length_map_[row][col] - 1 &&
               maze_.bottom_walls[row][col] == 0) {
      ++row;
    }
...
std::vector<SidePoint> PathFinder::GetVectorOfMoves_(int row, int col) {
  ...
  if (row > 0) {
    points.emplace_back(
        SidePoint{row - 1, col, maze_.bottom_walls[row - 1][col]});
  }
  if (col > 0) {
    points.emplace_back(
        SidePoint{row, col - 1, maze_.right_walls[row][col - 1]});
  }
