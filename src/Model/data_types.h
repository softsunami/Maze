 ...
 */
enum TypesOfEntities { kMaze = 1, kCave = 2 };

/**
 * @enum Limits
 * @brief An enumeration for the limits of the minimum and maximum size of the
 * maze and cave.
 */
enum class Limits {
  kMinOfRows = 0,
 ...
 */
enum Cell { kAliveCell = 1, kDeathCell = 0 };
...
struct BasicType<kMaze> final {
  ...
  bool IsValid() const {
    if (right_walls.empty() || bottom_walls.empty()) {
      return false;
    } else if (rows > static_cast<std::size_t>(Limits::kMaxOfRows) ||
               cols > static_cast<std::size_t>(Limits::kMaxOfCols)) {
      return false;
    }
